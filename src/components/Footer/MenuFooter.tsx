...
const MenuFooter: React.FC<MenuFooterProps> = ({ viewFooter }) => {
  const { isAddToCartLoading, addToCart, price, useLegacyScreenshot, product, setCameraByName, saveComposition,
		createQuote } = useZakeke();
  const { priceFormatter, bodyMeasurements, kneeSliders, isDraftEditor, isEditorMode,
  isQuoteLoading,
		setIsQuoteLoading,
		isViewerMode,
   } = useStore();
  ...
  return (
    <div className="menu_footer_master">
      ...
      {/* {product?.quoteRule && !isViewerMode && !isDraftEditor && !isEditorMode && !isDraftEditor && ( */}
					<div className="menu_actions">
          		<div  className="btn btn-primary menu_btn_cart"
